...
fig <- MobileSensorReadings_byloc %>%
  plot_ly(
    ...
    marker = list(
      color = ~hourly_Value, 
      size = 19, # size = ~hourly_Value, sizeref = 16, 
      opacity=.9,
      colorscale = list(c(0, "#a0c2fa"), c(1, "red")), showscale=T,
...
#---- Adjusting the map in plotly
# plot_ly(x = c(-119.9999, -119.7117), y = c(0.017057,0.238372)) %>%
# plot_ly(MobileSensorReadings_byloc, x = ~Long, 
#     y = ~Lat) %>%
#   layout(
#     images = list(
#       source = raster2uri(as.raster(img)),
